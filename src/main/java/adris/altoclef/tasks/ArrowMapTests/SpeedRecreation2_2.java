...
public class SpeedRecreation2_2 {
...
//
    ...
    private static double velFormular(final double t, final Axis axis, final double v_0, final double q, final double m) {
        if (axis.equals(Axis.X)) {
            return v_0 * Math.pow(q, t-1) - m * ((1-Math.pow(q,t)) / (1-q)); //FIXME: TODO: is still formular up-to-date?
        }
        return v_0 * Math.pow(q, t-1) + m * ((1-Math.pow(q,t)) / (1-q)); //FIXME: TODO: is still formular up-to-date?
    }
...
/*    double neglectableSpeedLimit(final double m, final double q, final double v_0, final Axis axis, final double endTick, final double walkedTicks) {
    ...
    double neglectableSpeedLimit(final double m, final double q, final double v_0, final Axis axis, final double endTick, final double walkedTicks) {
        ...
        msum += m;
        System.out.println("[ngs at " + acounter + ": " + msum + "]");
        final double formula = playerFormularXZ(m,q,v_0,axis,t, velocityState);


        final double v_0new = velFormular(t, axis, v_0, q, m);
        return formula + neglectableSpeedLimit(m, q, v_0new, axis, endTick, walkedTicks + t);
    ...
    private class SlipperinessSpot {
        final BlockPos pos;
        final Block block;
    ...
    private static double playerFormularXZ(final double m, final double q, final double v_0, final Axis axis, final double t, final VelocityState velocityState) {
        return axis.equals(Axis.X) ?
                !velocityState.equals(VelocityState.Middle) ?
                        1 / (1 - q) * (v_0 * (1 - Math.pow(q, t)) + m * (-t + ((1 - Math.pow(q,t+1))/(1-q)) - 1)) //FIXME: This old, use new formular here
                        :
                        -m * t
                :
                (!velocityState.equals(VelocityState.Middle)) ?
                        v_0 * ((1-Math.pow(q,t))/(1-q)) + ((m*t)/(1-q)) - ((m)/(1-q)) * ((1-Math.pow(q,t+1))/(1-q)) + m/(1-q)//FIXME: This old, use new formular here
                        :
                        m * t;
    }

    private static double getDiffPlayerVelVsLineVel(final double t, final Axis axis, final double v_0, final double q, final double m) {
        return velFormular(t, axis, v_0, q, m);
    }

    static final Bisection getPlayerVelVsLineVelBisection(final double t, final Axis axis, final double v_0, final double q, final double m) {
    ...
    private static double getDiffPlayerVsLineXZ(final double t, final Axis axis, final double v_0, final double q, final double m, final double xOrZOfLine) {
        final VelocityState velocityState = Double.compare(v_0, 0.003d) > 0 ? VelocityState.Positive :
                Double.compare(v_0, -0.003d) > 0 ? VelocityState.Middle :
                        VelocityState.Negative;
    ...
    static final Bisection getPlayerVsLineBisection(final double t, final Axis axis, final double v_0, final double q, final double m, final double xOrZOfLine) {
        return new Bisection() {
            @Override
            public double f(double t) {
    ...
    double slipperinessSplitter(final Vec3d start, final Vec3d end, final World world) {
        final BlockPos startBlock = new BlockPos(start);
        final BlockPos endBlock = new BlockPos(end);
        //Pass center of player
        final List<BlockPos> path = FloatingPointBresenham.floatingPointBresenham((int)start.getY(), start.getX(), start.getZ(), end.getX(), end.getZ());
        ...
        for (final SlipperinessSpot current : spots) {
            if (spots.indexOf(current) == spots.size() - 1 || !prev.equals(current)) {
                final Axis lineAsAxis = (prev.getPos().up().equals(current.getPos()) || prev.getPos().down().equals(current.getPos())) ? Axis.Z : Axis.X;

                if (lineAsAxis.equals(Axis.Z)) { // if line is like z axis, we look at where player.x cuts line.x
                    double line;
                    if (current.getPos().getX() < prev.getPos().getX()) {
                        line = prev.getPos().getX();

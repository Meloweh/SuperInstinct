 ...
 * Copyright (C) 2023 MelowehAndWelomeh
 */
public class BlindWalkWithSpeedTestTask2 extends Task {
    ...
    public BlindWalkWithSpeedTestTask2(final BlockPos target) {
        this.target = target;
        this.finished = false;
    }
    ...
    private boolean walkStepTowardsPoint(final AltoClef mod, final Vec3d target) {
        ...
        if (!isStarting) {
            ...
            final double mXXX = INPUT_CONSTANT * (ON_GROUND ? MOVEMENT_SPEED * (0.21600002F / (SLIPPERINESS * SLIPPERINESS * SLIPPERINESS)) : DEFAULT_FLYING_SPEED) * (double)MathHelper.sin(startYaw * 0.017453292F);
            System.out.println("MAYBE startVelX too?: " + ((-mXXX * q - mXXX)*q));
            ...
            final double mXX = CSAlgorithm.getM(Axis.X, startYaw, startPos, mod.getWorld(), mod.getPlayer().getBoundingBox(), true);

            if (Double.compare(mX, mXX) != 0) System.out.println("NEQ___M: " + mX + " vs " + mXX);

            int tC = tickCounter+++1;
            double vx1 = SpeedRecreation2.debugNonDeltaVelFormular(tC, Axis.X, startVel.x, q, mX);
            double vx2 = SpeedRecreation2.debugNonDeltaVelFormular(tC, Axis.X, 0, q, mXX);//mX * q + mX * Math.pow(q, 2)
            double vx3 = SpeedRecreation2.debugNonDeltaVelFormular(tC-1, Axis.X, -mX * q - mX * Math.pow(q, 2), q, mXX);
            double vxA = SpeedRecreation2_3.deltaVelFormular(tC-1, Axis.X,   ((-mXX * q - mXX) * q), q, mXX)*q;
            double vxB = SpeedRecreation2_3.velFormular(tC-1, Axis.X, 0, q, mXX);
            if (Double.compare(vx1, vx2) != 0) System.out.println("NEQ___VEL: " + vx1 + " vs " + vx2);
            if (Double.compare(vx1, vx3) != 0) System.out.println("NEQ___VEL3: " + vx1 + " vs " + vx3);
            ...
            System.out.println("traced velx for tick="+tickCounter+": " + vx1);
            System.out.println("traced velx2 for tick="+tickCounter+": " + vx2);
            System.out.println("traced velx3 for tick="+tickCounter+": " + vx3);
            System.out.println("traced velxA for tick="+tickCounter+": " + vxA);
            System.out.println("traced velxB for tick="+tickCounter+": " + vxB);
            System.out.println("vx: " + mod.getPlayer().getVelocity().getX() + " vy: " +" vz: " + mod.getPlayer().getVelocity().getZ());
     ...
     //       System.out.println("velx: " + mod.getPlayer().getVelocity().getX());
       ...
       //     System.out.println("POS5: " + pos5);
        ...
        if (!mod.getClientBaritone().getInputOverrideHandler().isInputForcedDown(Input.MOVE_FORWARD)) {
            ...
            //final double mXX = CSAlgorithm.getM(Axis.X, startYaw, startPos, mod.getWorld(), mod.getPlayer().getBoundingBox(), true);
            //final double withoutv0informular = mXX * Math.pow(q, 0+1) + 2*mXX * Math.pow(q, 0) - mXX * ((1-Math.pow(q, 0+1))/(1-q));
            //System.out.println("withoutv0informular: " + withoutv0informular);
            ...
            final double m = CSAlgorithm.getM(Axis.X, startYaw, mod.getPlayer().getPos(), mod.getWorld(), mod.getPlayer().getBoundingBox(), true);
            System.out.println("neglectableLimit2 for X next tick: " + SpeedRecreation2_3.neglectableSpeedLimitUsage(mod.getPlayer().getPos().x, m, q, Axis.X, 1, 0));
